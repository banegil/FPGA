...
int uart_lconf(enum UART port, struct ulconf *lconf)
{
	...
	// Configuración de longitud de palabra
	...
	// Configuración de número de bits de parada
	if(lconf->stopb == TWO)				// xxxx1xx
		confvalue |= 0x04;

	// Configuración del modo de paridad
	if(lconf->par == NONE)
		/* Intencionado */;
...
static void uart_rx_ready(enum UART port)
{
	switch (port)
	{
		case UART0:
			...
			while(!(rUTRSTAT0 & 0x01)) /* Intencionado */;
...
static void uart_tx_ready(enum UART port)
{
	switch (port)
	{
		case UART0:
			...
			while(!(rUTRSTAT0 & 0x02)) /* Intencionado */;
		...
		case UART1:
			...
			while(!(rUTRSTAT1 & 0x02)) /* Intencionado */;
