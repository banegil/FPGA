...
void Task7( void *id )
{
    ...
    while( 1 )                         /* Mostrar por LCD teclas pulsadas */
    {
    	scancode = *((uint8 *) OSQPend( keypadQueue, 0, &err ));
    	if(scancode != usedcode){

    	    		lcd_putint_x2( 20, 100, BLACK, scancode );
    	    		usedcode = scancode;
    	    	}
    }
}
...
void Task8( void *id )
{
    ...
    while( 1 )                         /* Muestra por LCD los segundos desde inicio */
    {
